...
// Enhancer function to extract basecall summary statistics
void extract_basecall_summary_stats(hid_t file_id, hid_t signal_dataset_id, fast5_metadata_t *metadata) {
  // Initialize fields
  metadata->median_template = 0.0;
  metadata->mad_template = 0.0;
  metadata->basecall_stats_available = false;

  if (signal_dataset_id < 0) return;

  // Suppress HDF5 error messages (many files won't have basecall data)
  H5E_auto2_t old_func;
  void *old_client_data;
  H5Eget_auto2(H5E_DEFAULT, &old_func, &old_client_data);
  H5Eset_auto2(H5E_DEFAULT, NULL, NULL);
  ...
  // Extract read path (parent of Signal dataset)
  char read_path[512];
  strncpy(read_path, obj_name, sizeof(read_path) - 1);
  read_path[sizeof(read_path) - 1] = '\0';
  char *last_slash = strrchr(read_path, '/');
  if (last_slash) *last_slash = '\0';

  // Try to open Analyses group
  char analyses_path[600];
  snprintf(analyses_path, sizeof(analyses_path), "%s/Analyses", read_path);

  hid_t analyses_group = H5Gopen2(file_id, analyses_path, H5P_DEFAULT);
  if (analyses_group < 0) {
    H5Eset_auto2(H5E_DEFAULT, old_func, old_client_data);
    return;
  }

  // Look for basecall summary groups (try multiple possible locations)
  const char *summary_groups[] = {
    "Basecall_1D_000/Summary/basecall_1d_template",
    "Basecall_1D_001/Summary/basecall_1d_template",
    "RawGenomeCorrected_000/BaseCalled_template",
    "Segmentation_000/Summary/split_hairpin_summary"
  };

  for (size_t i = 0; i < sizeof(summary_groups) / sizeof(summary_groups[0]); i++) {
    char summary_path[700];
    snprintf(summary_path, sizeof(summary_path), "%s/%s", analyses_path, summary_groups[i]);

    hid_t summary_group = H5Gopen2(file_id, summary_path, H5P_DEFAULT);
    if (summary_group >= 0) {
      // Try to read median_template and med_abs_dev_template
      if (read_double_attribute(summary_group, "median_template", &metadata->median_template) &&
          read_double_attribute(summary_group, "med_abs_dev_template", &metadata->mad_template)) {
        metadata->basecall_stats_available = true;
        H5Gclose(summary_group);
        break;
      }
      H5Gclose(summary_group);
    }
